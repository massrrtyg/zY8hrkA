...
function closeFromLevel(l) { 
    for(let i=openPanels.length-1; i>=l; i--) {
        if(openPanels[i]) { 
            openPanels[i].remove(); 
            openPanels[i]=null; 
        } 
...
function closeAllMenus(e) { 
    if(e && (e.target.closest('.menu-panel') || e.target.closest('#ctx-menu') || e.target.closest('#move-tool'))) return; 
        ...
        if (res.ok) {
            ...
            lines.forEach(line => {
                ...
                if (line.startsWith('pattern:')) {
                    if (current.pattern) {
                        iconRules.push({...current});
                        current = {};
                    }
                    const match = line.match(/pattern:\s*['"]?(.*?)['"]?$/);
                    if (match) current.pattern = match[1];
                } else if (line.startsWith('src:')) {
                    const match = line.match(/src:\s*['"]?(.*?)['"]?$/);
                    if (match) current.src = match[1];
                }
            });
            if (current.pattern && current.src) {
                iconRules.push(current);
            }
        }
    } catch (e) {
...
function resolveIcon(node) {
    const isF = 'children' in node;
    
    if (isF) {
        return {
            src: CUSTOM_FOLDER_ICON,
            errorAttr: `this.src='${FALLBACK_ICON}'`
        };
    }

    if (node.url && iconRules.length > 0) {
        for (const rule of iconRules) {
            try {
                const regex = new RegExp(rule.pattern);
                if (regex.test(node.url)) {
                    let finalSrc = rule.src;
                    if (!/^https?:\/\//i.test(finalSrc)) {
                        finalSrc = `/static/ico/${finalSrc}`;
                    }
                    return {
                        src: finalSrc,
                        errorAttr: `this.onerror=null;this.src='${FALLBACK_ICON}'`
                    };
                }
            } catch (e) {
            }
...
function getNode(p) { 
    let curr = getToolbarList(); 
    for(let i=0; i<p.length; i++){ 
        if(!curr) return {};
        if(i===p.length-1) return curr[p[i]] || {}; 
        curr=curr[p[i]].children; 
    } 
    return curr || {}; 
}

function backupOpenPanelPaths() {
    _openRefs = [];
    openPanels.forEach(p => {
        if (p && p.dataset.path) {
            const pathObj = JSON.parse(p.dataset.path);
            const nodeObj = getNode(pathObj);
            if (nodeObj && !Array.isArray(nodeObj)) {
                _openRefs.push({ panel: p, obj: nodeObj });
            }
        }
    });
}

function restoreOpenPanelPaths() {
    function findPath(list, target, acc) {
        for (let i = 0; i < list.length; i++) {
            if (list[i] === target) return [...acc, i];
            if (list[i].children) {
                const res = findPath(list[i].children, target, [...acc, i]);
                if (res) return res;
            }
        }
        return null;
    }
    const currentList = getToolbarList();
    _openRefs.forEach(ref => {
        const newPath = findPath(currentList, ref.obj, []);
        if (newPath) {
            ref.panel.dataset.path = JSON.stringify(newPath);
        } else {
...
function render() {
    ...
    measureDiv.style.cssText="position:absolute;visibility:hidden;display:flex;white-space:nowrap;";
    document.body.appendChild(measureDiv);
    const barWidth = bar.clientWidth - 40; 
    let curW = 0, overIdx = -1;
    list.forEach((n, i) => { 
        const temp = createItem(n, [i]); 
        measureDiv.appendChild(temp); 
        if (overIdx === -1 && curW + temp.offsetWidth + 2 > barWidth) overIdx = i; 
        curW += temp.offsetWidth + 2; 
    });
...
function createItem(node, path) {
    const isF = 'children' in node; 
    const el = document.createElement(isF?'div':'a'); 
    el.className='item'; 
    ...
    const iconData = resolveIcon(node);
    const safeTitle = escapeHtml(node.title);
    const label = hideText ? '' : `<span>${safeTitle}</span>`;
    
    el.innerHTML = `<div class="icon-wrapper"><img src="${iconData.src}" onerror="${iconData.errorAttr}"></div>${label}${isF && path.length>1?'<span class="arrow-right">▶</span>':''}`;
...
function onDrop(e) {
    ...
    if (dropPos === 'inside') {
        const destNode = getNode(targetPath);
        if (!destNode.children) destNode.children = [];
        destList = destNode.children;
        baseDestIdx = destList.length;
    } else {
        const destParentPath = targetPath.slice(0, -1);
        destList = destParentPath.length ? getNode(destParentPath).children : getToolbarList();
        const targetIdx = targetPath[targetPath.length - 1];
        baseDestIdx = dropPos === 'before' ? targetIdx : targetIdx + 1;
    }

    backupOpenPanelPaths();

    const itemToMove = srcList[srcIdx];
    srcList.splice(srcIdx, 1);

    let finalDestIdx = baseDestIdx;
    if (srcList === destList && baseDestIdx > srcIdx) {
        finalDestIdx--;
    }

    destList.splice(finalDestIdx, 0, itemToMove);
    
    dragSrcPath = null; 
    
    restoreOpenPanelPaths();
    save();
}
...
function openMenu(pEl, node, path) {
    ...
    if(!node.children?.length) panel.innerHTML='<div style="padding:10px 20px;opacity:0.5;">(空)</div>'; 
    else node.children.forEach((c, i) => panel.appendChild(createItem(c, [...path, i])));
    document.body.appendChild(panel); 
    openPanels[level-1]=panel;
    if(!isMobile()){
...
function showOverflowMenu(items, rect, start) { 
    ...
    items.forEach((it, i) => p.appendChild(createItem(it, [start+i]))); 
...
function openMoveTool(e) { 
    const tool = document.getElementById('move-tool'); 
    tool.style.display='flex'; 
    document.getElementById('move-tool-title').innerText = "移动: "+getNode(activePath).title; 
    if(!isMobile()){ 
        tool.style.left=e.pageX+'px'; 
...
function updateMoveButtons() { 
    const idx = activePath[activePath.length-1];
    const pP = activePath.slice(0,-1);
    const arr = pP.length?getNode(pP).children:getToolbarList();
    const isB = activePath.length===1; 
    document.getElementById('btn-up').className = (!isB && idx>0)?'move-btn':'move-btn disabled'; 
    document.getElementById('btn-down').className = (!isB && idx<arr.length-1)?'move-btn':'move-btn disabled'; 
    document.getElementById('btn-left').className = (isB && idx>0)?'move-btn':'move-btn disabled'; 
    document.getElementById('btn-right').className = (isB && idx<arr.length-1)?'move-btn':'move-btn disabled'; 
}

async function onMoveItem(dir) { 
    const idx = activePath[activePath.length-1];
    const pP = activePath.slice(0,-1);
    const arr = pP.length?getNode(pP).children:getToolbarList();
    const nI = idx + dir; 
    if(nI>=0 && nI<arr.length){ 
        ...
        activePath[activePath.length-1]=nI; 
        restoreOpenPanelPaths();
        render(); 
        updateMoveButtons(); 
        save(); 
...
function openModal(mode) {
    ...
    if(isSearch){ 
        document.getElementById('modal-title').innerText = "搜索书签"; 
        document.getElementById('inp-search-keyword').value = ''; 
        document.getElementById('search-results').innerHTML = ''; 
        setTimeout(() => document.getElementById('inp-search-keyword').focus(), 50); 
    }
    else {
        document.getElementById('modal-title').innerText = mode==='edit'?"编辑书签":mode==='addLink'?"添加网页":"添加文件夹";
        const fdr = document.getElementById('inp-folder'); 
        fdr.innerHTML='<option value="[]">根目录</option>'; 
        ...
        if(mode==='edit'){ 
            const n=getNode(activePath); 
            document.getElementById('inp-name').value=n.title; 
            document.getElementById('inp-url').value=n.url||''; 
            document.getElementById('url-field').style.display=n.children?'none':'block'; 
            document.getElementById('inp-folder').value = JSON.stringify(activePath.slice(0, -1)); 
        }
        else { 
            document.getElementById('inp-name').value=''; 
            document.getElementById('inp-url').value='https://'; 
            document.getElementById('url-field').style.display=mode==='addLink'?'block':'none'; 
            const curP = openPanels.find(p => p && p.style.display !== 'none'); 
            if (curP && curP.dataset.path) document.getElementById('inp-folder').value = curP.dataset.path; 
        }
    }
}
...
function confirmDialog() {
    ...
    const folderPath = JSON.parse(document.getElementById('inp-folder').value);
    ...
    if (modalMode === 'edit') { 
        ...
        if (JSON.stringify(oldPath) !== JSON.stringify(folderPath)) { 
            const oldIdx = activePath[activePath.length-1];
            const oldParent = oldPath.length ? getNode(oldPath).children : getToolbarList(); 
...
function onToggleIconMode() { 
    const n=getNode(activePath); 
    n.icon_only=!n.icon_only; 
    save(); 
    closeAllMenusInternal(); 
}
...
function refreshFloatingPanels() { 
    openPanels.forEach(p=>{ 
        if(p && p.dataset.path && p.style.display !== 'none'){ 
            const path=JSON.parse(p.dataset.path);
            const n=getNode(path); 
            p.innerHTML=''; 
            if(n && n.children){ 
                if(n.children.length === 0) {
                    p.innerHTML='<div style="padding:10px 20px;opacity:0.5;">(空)</div>';
                } else {
                    n.children.forEach((c,i)=>p.appendChild(createItem(c,[...path,i]))); 
                }
            } else {
...
searchInput.oninput = e => {
    ...
    const find = (ns, pathStack = []) => ns.forEach(n => { 
        if(n.url && (n.title.toLowerCase().includes(kw)||n.url.includes(kw))){ 
            ...
            a.innerHTML = `<img src="${iconData.src}" onerror="${iconData.errorAttr}" style="width:18px;height:18px;margin-right:12px;border-radius:3px;"><div class="search-info"><div style="font-weight:500;">${safeTitle}</div><div class="search-url">${safeUrl}</div><div class="search-path">位置: ${safePath}</div></div>`; 
            res.appendChild(a); 
        } 
    ...
    try { 
        ...
        if(res.ok) { 
            ...
            if(notesData.length > 0) { 
                if (!activeNoteId || !notesData.some(n => n.id === activeNoteId)) activeNoteId = notesData[0].id; 
            } else {
                addNewNote(false); 
            }
            renderNotes(); 
...
async function saveNotes(silent = false) {
    if(!activeNoteId || !NOTE_CONF.enabled) return; 
    const noteIdx = notesData.findIndex(n => n.id === activeNoteId); 
    if(noteIdx !== -1 && NOTE_TEXTAREA) { 
        notesData[noteIdx].content = NOTE_TEXTAREA.value; 
        notesData[noteIdx].timestamp = Date.now(); 
    }
    if (!silent) showLoading();
    try { 
        await fetch('/api/save_notes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(notesData) }); 
    } catch (e) {
    } finally { 
...
function autoSaveNote() { 
    ...
    saveTimer = setTimeout(() => saveNotes(true), 1000); 
}
...
function renderNotes() {
    ...
    if (activeNote) NOTE_TEXTAREA.value = activeNote.content; 
    else { 
        activeNoteId = notesData[0].id; 
        NOTE_TEXTAREA.value = notesData[0].content; 
    }
    notesData.forEach((note, idx) => { 
        const tab = document.createElement('div'); 
        tab.className = 'note-tab' + (note.id === activeNoteId ? ' active' : ''); 
        tab.innerHTML = `<span>笔记 ${idx + 1}</span> <span class="note-tab-close" onclick="openDeleteNoteModal(${note.id}, event)">x</span>`; 
        tab.onclick = (e) => { e.stopPropagation(); switchNote(note.id); }; 
        NOTE_TABS.appendChild(tab); 
    });
    NOTE_TEXTAREA.focus();
}

function switchNote(id) { 
    const oldIdx = notesData.findIndex(n => n.id === activeNoteId); 
    if(oldIdx !== -1) { 
        notesData[oldIdx].content = NOTE_TEXTAREA.value; 
        notesData[oldIdx].timestamp = Date.now(); 
    } 
    activeNoteId = id; 
    renderNotes(); 
}

function addNewNote(shouldSave = true) { 
    if (shouldSave) saveNotes(); 
    const newNote = { id: generateUniqueId(), content: '', timestamp: Date.now() }; 
...
function openDeleteNoteModal(id, e) { 
    e.stopPropagation(); 
    noteToDeleteId = id; 
    closeAllMenusInternal(); 
    document.getElementById('modal-overlay').style.display = 'flex'; 
    document.getElementById('search-container').style.display = 'none'; 
    document.getElementById('modal-title').innerText = "删除笔记"; 
    const f = document.getElementById('form-fields'); 
    f.style.display = 'block'; 
    f.innerHTML = `<p style="color:var(--text-dim); padding:10px 0;">永久删除这条笔记吗？</p>`; 
    const b = document.getElementById('btn-confirm'); 
    b.innerText = "确认删除"; 
    b.style.backgroundColor = '#ff6b6b'; 
    b.style.color = '#fff'; 
    b.onclick = deleteNoteConfirmed; 
}
...
function toggleNotePanel(e) { 
    e.stopPropagation(); 
    closeAllMenusInternal(); 
    if (NOTE_PANEL.style.display === 'flex') { 
        NOTE_PANEL.style.display = 'none'; 
        saveNotes(); 
    } else { 
        loadNotes(); 
        NOTE_PANEL.style.display = 'flex'; 
    } 
}
